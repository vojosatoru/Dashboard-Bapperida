...
def render_tables():
    # --- FITUR EKSPOR / IMPOR PROYEK OFFLINE & CLOUD GIST ---
    with st.expander("💾 Simpan / Muat Proyek (.json)", expanded=False):
        ...
        # --- TAB MANAJEMEN DATA ---
        tab_lokal, tab_cloud = st.tabs(["💻 Penyimpanan Lokal", "☁️ Cloud Backup (1-Click Sync)"])
        ...
        with tab_cloud:
            ...
            try:
                ...
                pass # Abaikan secara diam-diam jika file secrets.toml tidak ada (misal run lokal biasa)
            ...
            # AKSI 1: PUSH (Hanya muncul jika token dan gist ada)
            ...
            # AKSI 2: PULL (Hanya muncul jika token dan gist ada)
            if col_pull.button("📥 PULL (Muat Data dari Cloud)", use_container_width=True, disabled=(not gh_token or not gh_gist)):
                with st.spinner(f"Sedang mengunduh data {current_key.capitalize()} dari Cloud..."):
                    ...
                    if sukses:
                        try:
                            # Mengekstrak isi file dari respon Gist API
                            ...
                            if nama_file_unduh not in files_dict:
                                ...
                                st.success(f"✅ Data Ruang '{current_key.capitalize()}' berhasil dipulihkan dari Cloud!")
                                import time
                                time.sleep(1)
                                st.rerun()
                        except Exception as e:
